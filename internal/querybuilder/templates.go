...
var (
	...
	testTemplate                  = template.Must(template.New("ct-tests").Funcs(funcMap).Parse(tests))
)
...
func Test{{.ModelName}}QueryBuilder(t *testing.T) {
    {{ $index, $field := range .Fields }}
	t.Run("where by {{$field.Name}} limit 1", func(t *testing.T) {
		q := {{$.ModelName}}QueryBuilder().Limit(1).Where{{$field.Name}}Eq($.FieldsSampleValues[$index])
		query, err := q.SQL()
		assert.NoError(t, err)
		assert.Equal(t, "SELECT * FROM users WHERE {{ toSnakeCase $field.Name }} = ? LIMIT 1", query)
		assert.Equal(t, []any{$.FieldsSampleValues[$index]}, q.(*__UserSQLQueryBuilder).whereArgs)
	})
    {{ end }}
}
