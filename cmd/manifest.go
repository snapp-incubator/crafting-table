...
func apply(_ *cobra.Command, _ []string) {
	...
	for _, repo := range manifest.Repos {
		...
		if err := repository.Generate(repo.Source, repo.Destination, repo.PackageName, repo.StructName, &repo.Get, &repo.Update, repo.Join, repo.Create.Enable, repo.Test); err != nil {
			log.Fatal(err)
		}
	}
}
